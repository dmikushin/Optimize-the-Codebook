...
double **readUniverseFile(const char *fileName, int universeSize, int p,
                          int rowSize) {
  char *line;
...
double **KMeans(double **universe, int universeSize, double **codebook, int K,
                int p, double delta, double w[], FILE *consoleFile) {
  int M = 0, minIndex = 9999999; // minIndex stores the index of the code vector
  ...
  double previousDistortion = 0.0, currentDistortion = 1000.0,
         min = 1000000000.0, totalDistortion = 0.0;
  long long int iterationCount = 0; // total number of iterations in kmeans
  /*bucketNumber stores values from 0 to N-1 included i.e. the respective bucket
  numbers of all the vectors in the universe bucketSize stores the number of x
  vectors of each bucket*/
  int *bucketNumber = new int[universeSize],
      *bucketSize = new int[K];         // should lie between 0 to K-1 included
  ...
  while (abs(currentDistortion - previousDistortion) >=
         delta) // till the difference is more than the threshold
  {
    ++iterationCount;
    totalDistortion = 0;
    for (i = 0; i < K; ++i)
    ...
    for (i = 0; i < universeSize; ++i) // for each vector in the universe
    {
      ...
      for (j = 0; j < K; ++j) // for each vector in the codebook
      {
        ...
        {
          distances[j] +=
              w[k] * (universe[i][k] - codebook[j][k]) *
              (universe[i][k] - codebook[j][k]); // find out the distance
        }
      }
...
double **LBG(double **universe, int universeSize, double *initialCodebook,
             int K, int p, double delta, double epsilon, double w[]) {
  ...
  while (codebookSize != K) // till the desired size is not reached
  {
    ...
    codebook = KMeans(universe, universeSize, codebook, codebookSize, p, delta,
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  double w[] = {1.0,  3.0,  7.0,  13.0, 19.0, 22.0,
                25.0, 33.0, 42.0, 50.0, 56.0, 61.0}; // given Tokhura weights
  ...
  universe = readUniverseFile(universeFileName, universeSize, p,
                              rowSize); // get the universe file
